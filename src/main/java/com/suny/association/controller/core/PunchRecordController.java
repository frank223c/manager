...
public class PunchRecordController extends BaseController {
    ...
    public ResultDTO update(HttpServletRequest request, @RequestParam("punchMemberId") Integer punchMemberId) {
        ...
        if (member == null) {
            ...
            return ResultDTO.failureResult(LoginEnum.NO_LOGIN_IN);
        }
            ...
            if (databaseMember == null) {
                ...
                if (punchRecord != null) {
                    if (!punchRecord.getPunchTypeId().getPunchTypeId().equals(0)) {
                        ...
                        return ResultDTO.failureResult(PunchOperateEnum.REPEAT_PUNCH);
                    } else {
                        ...
                        if (successRow == 0) {
                            ...
                            return ResultDTO.successResult(PunchOperateEnum.PUNCH_FAIL);
                        }
                    }
                } else {
                        /*   系统设计是考勤当天首先由管理员开始考勤，然后系统自动给每一个成员添加一条缺勤记录   */
                    logger.warn("213,管理员还没有开启签到！");
                    return ResultDTO.failureResult(PunchOperateEnum.TIME_NOT_REACH);
                }
            }
        }
        logger.info("214,{}考勤成功", member.getMemberName());
        return ResultDTO.successResult(PunchOperateEnum.PUNCH_SUCCESS);
    }
    ...
    public ResultDTO insert(HttpServletRequest request, @RequestParam("memberId") Long memberId) {
        ...
        if (sessionMember == null) {
            ...
            if (dataBaseMember == null) {
                ...
                return ResultDTO.failureResult(CommonEnum.LIMIT_MEMBER_MANAGER);
                ...
                if (successRow == 0) {
                    ...
                    return ResultDTO.failureResult(PunchOperateEnum.OPEN_PUNCH_FAIL);
                }
                return ResultDTO.successResult(PunchOperateEnum.OPEN_PUNCH_SUCCESS);
            }
        }
    }
